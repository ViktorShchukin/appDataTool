...
class DistanseCalculator:
        ...
        match data.sensorType:
            ...
            case "android.sensor.linear_acceleration":
                if self.isFirstDataAcc is True:
                    ...
                    self.listAcceleration[1].append(self.lastAccelerationData.timestamp)
                    ...
                    return
                self.evalueteAcceleration(self.lastAccelerationData, self.rotate(data))
                self.lastAccelerationData = self.rotate(data)
                self.listAcceleration[0].append(self.lastAccelerationData.y)
                self.listAcceleration[1].append(self.lastAccelerationData.timestamp)
                self.listAcceleration[2].append(self.lastAccelerationData.x)
    ...
    def integral(self, valueBefore: int, valueLast: int, timestampBefore: int, timestampLast: int):
        delta = (valueLast + valueBefore)/2 * (timestampLast - timestampBefore)/1e9
        return delta

    def evalueteGyroscope(self, dataBefore: SensorData, dataLast: SensorData):
        delta_x = self.integral(dataBefore.x, dataLast.x, dataBefore.timestamp, dataLast.timestamp)
        delta_y = self.integral(dataBefore.y, dataLast.y, dataBefore.timestamp, dataLast.timestamp)
        delta_z = self.integral(dataBefore.z, dataLast.z, dataBefore.timestamp, dataLast.timestamp)
    ...
    def evalueteAcceleration(self, dataBefore: SensorData, dataLast: SensorData):
        ...
        delta_velocity_x = self.integral(dataBefore.x, dataLast.x, dataBefore.timestamp, dataLast.timestamp)
        delta_velocity_y = self.integral(dataBefore.y, dataLast.y, dataBefore.timestamp, dataLast.timestamp)
        delta_velocity_z = self.integral(dataBefore.z, dataLast.z, dataBefore.timestamp, dataLast.timestamp)

        delta_distance_x = self.integral(self.lastVelosity[0], self.lastVelosity[0] + delta_velocity_x, dataBefore.timestamp, dataLast.timestamp)
        delta_distance_y = self.integral(self.lastVelosity[1], self.lastVelosity[1] + delta_velocity_y, dataBefore.timestamp, dataLast.timestamp)
        delta_distance_z = self.integral(self.lastVelosity[2], self.lastVelosity[2] + delta_velocity_z, dataBefore.timestamp, dataLast.timestamp)
...
def mult_matrix(a: list[list], b: list[list]):
    if len(a[0]) != len(b):
        raise ArithmeticError("cannot multiplicate matrix because of different row and kolumn number")
    newMatrix = list()
...
if __name__ == '__main__':
    ...
    dataList = accessdata.get_data_row(name="row_data_gyro_straight_1_1")
    ...
    velocity_y = dp.get_velocity(cal.listAcceleration[0], cal.listAcceleration[1])
